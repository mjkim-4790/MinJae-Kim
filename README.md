...
#### 카메라·센서 게임을 폰에서 테스트할 때는 HTTPS 로 띄운다

카메라(`getUserMedia`)와 기울기·흔들림 센서는 **보안 컨텍스트**에서만 동작한다.
`http://localhost` 는 예외로 인정되지만 폰이 접속하는 `http://192.168.x.x` 는 아니라서,
그냥 두면 색깔 사냥·미로·무궁화가 폰에서 켜지지 않는다. 그럴 때만 켜면 된다:

```bash
HTTPS=1 npm run dev
```

- 개발 서버가 자체서명 인증서로 `https://localhost:5173` 과 `https://192.168.x.x:5173` 을 연다.
- 참여 QR 도 자동으로 `https://` 주소로 바뀐다 (`client/vite.config.js`).
- 서버 CORS 도 개발 모드에서 `https://<LAN IP>:5173` 을 함께 허용한다 (`server/src/config.js`).
- **폰에서 처음 한 번 "안전하지 않음" 경고를 지나야 한다.** 자체서명이라 정상이다.

기본값을 끔으로 둔 이유: 노트북에서만 볼 때는 `localhost` 로도 카메라가 켜지는데,
HTTPS 를 항상 켜두면 열 때마다 인증서 경고를 넘겨야 해서 번거롭다.
...
4. 운영자 계정을 만든다. **무료 티어는 Shell 탭 자체가 막혀 있어서**(Starter 플랜부터
   ...
   건너뛴다). 무료 티어는 재배포마다 DB 가 초기화되므로(아래 항목 참고) 이
   환경변수들은 지우지 말고 계속 켜둔다 — 그래야 재배포 후에도 같은 계정으로
   로그인할 수 있다.

   **운영자를 여러 명 두려면** `_2`, `_3` 처럼 번호를 붙인 키를 추가로 넣는다
   (`BOOTSTRAP_OPERATOR_EMAIL_2` / `_PASSWORD_2` / `_NAME_2`, 그다음은 `_3` ...).
   번호가 끊기는 지점에서 스캔을 멈추므로 중간을 비우지 않는다.
