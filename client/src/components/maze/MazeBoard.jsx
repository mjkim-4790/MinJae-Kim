...
import { START, ballRadiusFor, decodeMaze, reachedGoal, stepBall, E, N, S, W } from '../../lib/maze.js';
...
export default function MazeBoard({
  ...
  difficulty, // 'normal' | 'hard' — 공 크기가 다르다
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      const cell = Math.max(12, Math.floor(availW / cols));
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.beginPath();
      ctx.arc(b.x * cell, b.y * cell, radius * cell, 0, Math.PI * 2);
    ...
    const loop = (now) => {
      ...
      if (resetOnWall && now < frozenUntilRef.current) {
        ...
        const next = stepBall(ballRef.current, axis, cells, cols, rows, dt, radius);
      ...
      if (!goalHitRef.current && reachedGoal(ballRef.current, cols, rows)) {
  ...
  }, [cells, size, running, axisRef, ghostX, ghostY, resetOnWall, cols, rows, radius]);
